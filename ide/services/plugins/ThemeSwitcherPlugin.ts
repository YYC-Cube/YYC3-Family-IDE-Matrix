 ...
 * @migrated: 回迁自 archive/ide-monolith-2026-03/plugins/ThemeSwitcherPlugin.ts（第二批 · 2026-08-20） · 移除 @ts-nocheck
 */
 ...
 * 显示主题选择器
 */
function showThemeSelector(context: PluginAPI) {
 ...
 * 获取当前主题
 */
function getCurrentTheme(context: PluginAPI): string {
 ...
 * 加载保存的主题
 */
function loadSavedTheme(context: PluginAPI) {
 ...
 * 循环切换主题
 */
function cycleTheme(context: PluginAPI, direction: number) {
  const currentTheme = getCurrentTheme(context);
 ...
 * 应用主题
 */
function applyTheme(context: PluginAPI, themeId: string) {
