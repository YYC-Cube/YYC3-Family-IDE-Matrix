 ...
 * @migrated: 回迁自 archive/ide-monolith-2026-03/plugins/FileExplorerPlusPlugin.ts（第二批 · 2026-08-20） · 移除 @ts-nocheck
 */
 ...
 * 切换书签
 */
function toggleBookmark(context: PluginAPI) {
 ...
 * 显示书签列表
 */
function showBookmarks(context: PluginAPI) {
  const bookmarks = loadBookmarks();
 ...
 * 显示最近文件
 */
function showRecentFiles(context: PluginAPI) {
  const recent = loadRecentFiles();
 ...
 * 快速文件搜索
 */
function quickFileSearch(context: PluginAPI) {
