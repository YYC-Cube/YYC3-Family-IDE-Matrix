...
export interface SnapshotViewControllerConfig {
  ...
  /** 批量视图变化回调（推荐使用，减少重渲染） */
  onViewsChange?: (changes: Array<{ viewId: string; state: ViewState }>) => void;
  /** 同步状态变化回调 */
...
export class SnapshotViewController {
  ...
  constructor(config?: SnapshotViewControllerConfig) {
    ...
    this.syncScroll = this.config.comparisonConfig?.syncScroll ?? true;
    this.syncZoom = this.config.comparisonConfig?.syncZoom ?? true;
  ...
  /**
   * 批量通知视图变化
   * 优先使用 onViewsChange 批量回调以减少重渲染
   */
  private notifyViewsChanges(changes: Array<{ viewId: string; state: ViewState }>): void {
    if (changes.length === 0) {
      return;
    }

    if (this.config.onViewsChange) {
      this.config.onViewsChange(changes);
    }

    if (this.config.onViewChange) {
      for (const change of changes) {
        this.config.onViewChange(change.viewId, change.state);
      }
    }
  }
  ...
  private syncScrollPosition(sourceViewId: string, previousPosition: ViewPosition): void {
    ...
    const changes: Array<{ viewId: string; state: ViewState }> = [];
    ...
    this.views.forEach((view, viewId) => {
      if (viewId !== sourceViewId && view.visible) {
        ...
        changes.push({ viewId, state: { ...view } });
      }
    });

    this.notifyViewsChanges(changes);
  }
