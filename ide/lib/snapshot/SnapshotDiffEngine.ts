...
export class SnapshotDiffEngine {
   ...
   * 比较两个快照
   *
   * K-修复: Snapshot 接口允许 id 在"顶层"或 metadata.id 两种位置，
   * 测试用的 createSnapshot 工厂使用顶层 { id, label, ... }，
   * 而 engine 内部原本只认 metadata.id → undefined 导致 3 个断言失败。
   * 现在兼容两种：metadata?.id ?? snapshot.id ?? ''
   */
  public compareSnapshots(oldSnapshot: Snapshot, newSnapshot: Snapshot): SnapshotDiffResult {
    ...
    // 兼容性取值：优先 metadata.id，其次顶层 id，兜底空字符串
    const oldId = (oldSnapshot as any).metadata?.id ?? (oldSnapshot as any).id ?? '';
    const newId = (newSnapshot as any).metadata?.id ?? (newSnapshot as any).id ?? '';
