 ...
 * @migrated: 回迁自 archive/ide-monolith-2026-03/services/AIDegradationService.ts（1.5 批能力域回迁 · 2026-08-20）
 */
...
// DegradationLevel 定义已前移至 ./types（保持原 API 出口不变）
export type { DegradationLevel } from "./types";
...
export class AIDegradationService {
  ...
  private selectModel(config: ProviderConfig): string | null {
    // 回迁修复：原实现在 models.length===0 时提前返回 null，而 ollama 的预设模型
    // 列表为空（运行时检测），导致降级链中的 ollama 永远选不出模型、被 execute
    // 跳过。正确顺序：先查用户活跃模型，再回退预设列表，两者皆无才返回 null。
