 ...
 * @migrated: 回迁自 archive/ide-monolith-2026-03/plugins/QuickFixPlugin.ts（第二批 · 2026-08-20） · 移除 @ts-nocheck
 */
 ...
 * 扫描并修复
 */
function scanAndFix(context: PluginAPI) {
 ...
 * 显示问题面板
 */
function showIssuesPanel(context: PluginAPI, issues: Array<{
  type: string;
  severity: string;
  line: number;
  message: string;
 ...
 * 修复所有问题
 */
function fixAllIssues(context: PluginAPI) {
  const activeFile = context.editor.getActiveFile();
