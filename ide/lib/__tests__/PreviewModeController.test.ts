...
describe("PreviewModeController", () => {
  ...
  describe("realtime mode", () => {
    ...
    it("节流窗口内的多次变更应合并为尾部一次更新（v1.2.0 契约）", () => {
      controller.handleFileChange(); // 窗口外首帧：立即更新
      expect(mockTriggerUpdate).toHaveBeenCalledTimes(1);

      controller.handleFileChange(); // 进入 100ms 节流窗口
      controller.handleFileChange(); // 同窗口内合并
      expect(mockTriggerUpdate).toHaveBeenCalledTimes(1);

      vi.advanceTimersByTime(100); // 窗口结束，尾部更新触发
      expect(mockTriggerUpdate).toHaveBeenCalledTimes(2);

      vi.advanceTimersByTime(100); // 离开节流窗口
      controller.handleFileChange(); // 恢复立即更新
