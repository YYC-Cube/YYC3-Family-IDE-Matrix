 ...
 * @migrated: 回迁自 archive/ide-monolith-2026-03/plugins/AIAssistantPlugin.ts（第二批 · 2026-08-20） · 移除 @ts-nocheck
 */
 ...
 * 打开 AI 助手
 */
function openAssistant(context: PluginAPI) {
 ...
 * 解释代码
 */
function explainCode(context: PluginAPI) {
  const selectedText = context.editor.getSelectedText();
 ...
 * 优化代码
 */
function optimizeCode(context: PluginAPI) {
  const activeFile = context.editor.getActiveFile();
  const selectedText = context.editor.getSelectedText();
