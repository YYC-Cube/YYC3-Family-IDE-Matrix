 ...
 * @migrated: 回迁自 archive/ide-monolith-2026-03/PluginSystem.ts（第二批 · 2026-08-20）
 */
...
export interface PluginUIAPI {
  ...
  /** 兼容别名：归档插件普遍调用 showToast（原靠 @ts-nocheck 掩盖类型漂移） */
  showToast(
    message: string,
    type?: "info" | "success" | "warning" | "error",
  ): void;
  /** 兼容别名：归档插件调用 showPanel —— 经事件总线广播，由宿主 UI 决定呈现 */
  showPanel(
    idOrOptions:
      | string
      | { title: string; content: unknown; width?: number; height?: number },
    content?: unknown,
  ): void;
}
  ...
  private createPluginAPI(pluginId: string): PluginAPI {
    ...
    return {
      ...
      ui: {
        ...
        showPanel: (idOrOptions, content) => {
          const payload =
            typeof idOrOptions === "string"
              ? { id: idOrOptions, content, pluginId }
              : { id: idOrOptions.title, ...idOrOptions, pluginId };
          pluginSystem.eventBus.emit("panel:show", payload);
        },
      },
      ai: {
