 ...
 *
 * brief: 回迁自 archive/ide-monolith-2026-03/services/MCPPrompts.ts（第一批能力域回迁）
 *
 * details（回迁时修复）:
 * - 移除 @ts-nocheck，消息类型经 as PromptMessage[] 收敛（JSON-RPC 层为宽类型 string）
 * - 修复单引号包裹 ${} 不插值的模板字符串 bug（5 处 → 反引号）
 * - 依赖路径适配：./Logger → ../logger
 */
...
export class MCPPromptManager {
  ...
  async getPrompt(name: string, args?: Record<string, string>): Promise<PromptMessage[]> {
    ...
    // JSON-RPC 返回 role/content.type 为宽类型 string，在边界收敛为 PromptMessage 联合类型
